...
We will develop this method through three Qwen3 experiments in which **we deliberately injected errors** into generation-score handling, trainer-score handling, and weight synchronization. Because we know what changed, we can test whether the comparisons lead us back to it. The measured outputs show how to read the evidence, and the inline snippets show how to collect equivalent measurements in your own system. We begin with what the difference actually measures.
...
We now have three scores for each saved response token:
...
![A records generation; B uses rollout prefill and C uses a frozen trainer forward on the saved trajectory.](01-three-scores.png)

*Keep A unchanged. Neither B nor C generates a new response.*
...
Even with the correct saved sequence, extraction can pair a score with the wrong token. A causal model's output at one position predicts the token at the next position. In particular, the prediction **after the last prompt token** scores the first response token:

![The prediction after P2 scores R1; the prediction after R1 scores R2.](02-token-alignment.png)

*P1/P2 are prompt tokens; R1/R2 are response tokens.*
...
We can now use the A/B/C pattern to choose an investigation. The map below starts with the repeat checks because every later decision depends on having measurements stable enough to interpret:

![A decision map checks repeatability before routing generation, replay, or live-loop discrepancies.](03-debugging-map.png)

*Use the pattern to choose a targeted check, then verify the suspected cause.*
...
The response extraction follows the earlier alignment rule. Starting at `prompt_length - 1` accommodates the initial unscored input entry, then taking the response tail and checking its IDs ensures each B score belongs to the intended target.
